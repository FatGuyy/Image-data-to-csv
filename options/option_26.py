"""
This file gets the 26th option. (back to one name only)
"""
import os
...
def write_list_to_csv_column(files, csv_folder_path):
    pd.DataFrame({'product_name':files[0],
                ...
                'column f':files[5],
                'column g':files[6],
                'name':files[7]}).to_csv(os.path.join(csv_folder_path,'RP porn TS REG .csv'),index=False)
...
def option_26(FILE_NAMES, csv_folder_path, inventory_csv_path):
    ...
    product_name = "{} {} autographed TS Model RP 8x10 Photo RP{}"
    ...
    for i in FILE_NAMES:
        ...
        #sku list
        sku = First_name + Last_name[0] + f"{Last_4}" + date1 + "RPRts"
        sku_list.append(sku)
    ...
    column_g = col_g(colData, name_list, inventory_csv_path)
...
def option_26_3rd_csv(FILE_NAMES, inventory_csv_path):
    column_a = []
    column_b = []
    sku_2 = []
    colData = read_csv(inventory_csv_path) # read inventory
