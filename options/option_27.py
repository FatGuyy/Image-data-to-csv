"""
This file gets the 27th option. (back to one name only)
"""
import os
...
def write_list_to_csv_column(files, csv_folder_path):
    pd.DataFrame({'product_name':files[0],
                ...
                'column f':files[5],
                'column g':files[6],
                'name':files[7]}).to_csv(os.path.join(csv_folder_path,'RP porn TS REG boob.csv'),index=False)

    print("csv written...")

def option_27(FILE_NAMES, csv_folder_path):
    product_list  = []
    sku_list = []
    price = []
    stock = []
    ...
    product_name = "Model {} {} autographed TS RP 8x10 Photo RP{}"
    date1 = _date()
    ...
    for i in FILE_NAMES:
        ...
        #sku list
        sku = First_name + Last_name[0] + f"{Last_4}" + date1 + "RPRtsb"
        sku_list.append(sku)
...
def option_27_3rd_csv(FILE_NAMES):
    column_a = []
    column_b = []
