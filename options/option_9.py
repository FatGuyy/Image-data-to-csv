...
def write_list_to_csv_column(files, csv_folder_path):
    pd.DataFrame({'product_name':files[0],
                ...
                'name-link':files[7],
                'name2':files[8],
                'name2link':files[9]}).to_csv(os.path.join(csv_folder_path, 'tsxc Combo.csv'), index=False)
    print("csv written...")


def option_9(FILE_NAMES, csv_folder_path):
    product_list  = []
    sku_list = []
    price = []
    Sale_price = []
    ...
    for i in FILE_NAMES:
        ...
        #sku list
        sku = First_name + Last_name[0] + f"{Last_4}" + date1 + "tsxc"  # The end of sku
        sku_list.append(sku)
